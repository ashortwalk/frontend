...
export default function PostCommentContent() {
    ...
    try {
      const commentwite = async () => {
        const response = await axios.post(
          `https://20.41.86.171:8000/api/posts/${postId}/comments`,
          { content },
          {
            headers: {
  ...
  useEffect(() => {
    ...
    const findUser = async () => {
      const response = await axios.get(`https://20.41.86.171:8000/api/users`, {
  ...
  const saveEditing = async (id) => {
    try {
      const response = await axios.patch(
        `https://20.41.86.171:8000/api/posts/${postId}/comments/${id}`,
                          ...
                          <button
                            onClick={async () => {
                              if (window.confirm("댓글을 삭제하시겠습니까?")) {
                                try {
                                  await axios.delete(
                                    `https://20.41.86.171:8000/api/posts/${postId}/comments/${comm.id}`,
                                    {
                                      headers: {
                                        Authorization: authorization,
                                      },
